...
class WialonError(Exception):
    ...
    def __unicode__(self):
        ...
        if (self._code in WialonError.errors):
            explanation = " ".join(
                [WialonError.errors[self._code], self._text])

        message = u'{error} ({code})'.format(
            error=explanation, code=self._code)
...
class Wialon(object):
    ...
    def call(self, action_name, *argc, **kwargs):
        ...
        print(params)
        print(self.__default_params['params'])
    ...
    def request(self, action_name, url, params):
        ...
        if isinstance(result, list):
            ...
            for elem in result:
                ...
                if "error" in elem:
                    errors.append("%s (%d)" % (
                        WialonError.errors[elem["error"]], elem["error"]))
